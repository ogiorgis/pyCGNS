...
def makeStTree(vertexsize, cellsize):
    ...
    g = CGL.newGridCoordinates(z, 'GridCoordinates')
    CGL.newDataArray(g, CGK.CoordinateX_s, NPY.ones(tuple(vertexsize), dtype='float64', order='F'))
    CGL.newDataArray(g, CGK.CoordinateY_s, NPY.ones(tuple(vertexsize), dtype='float64', order='F'))
    CGL.newDataArray(g, CGK.CoordinateZ_s, NPY.ones(tuple(vertexsize), dtype='float64', order='F'))
    return (T, b, z)
